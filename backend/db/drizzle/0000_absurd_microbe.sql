...
CREATE TABLE "pkce_challenges" (
	...
	"expires_at" timestamp NOT NULL,
	"created_at" timestamp DEFAULT now(),
	CONSTRAINT "pkce_challenges_state_unique" UNIQUE("state")
);
--> statement-breakpoint
...
ALTER TABLE "pkce_challenges" ADD CONSTRAINT "pkce_challenges_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "pkce_challenges_state_idx" ON "pkce_challenges" USING btree ("state");--> statement-breakpoint
CREATE INDEX "pkce_challenges_expires_at_idx" ON "pkce_challenges" USING btree ("expires_at");--> statement-breakpoint
